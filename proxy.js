...
// Global rate limit via Deno KV when a DB is attached; fall back to an in-memory
// (per-isolate) limiter if KV is unavailable, so the app always boots regardless.
let kv = null;
try { kv = await Deno.openKv(); } catch { /* no KV attached; use in-memory */ }
...
Deno.serve(async (req) => {
    ...
    if (await rateLimited(ip)) return json({ error: "rate limited, try again shortly" }, 429);
